...
if(isset($_POST['submit'])){
	...
	if(!empty($work_name) && !empty($work_catagory) ){
		$work_insert = $dbcon->query("INSERT INTO my_best_artikel (works_name,catagory) VALUES('$work_name','$work_catagory')");
		if($work_insert){
			...
			if(!empty($photo)){
				...
				if($_FILES['photo']['size']<2000000){
					...
					if( in_array(strtolower($photo_ext),$accepted_extension_list) ){
						...
						// upload photo
						$update_photo =$dbcon->query("UPDATE my_best_artikel SET photo='$photo_name' WHERE id=$last_id");


						move_uploaded_file($_FILES['photo']['tmp_name'], 'image/my_best_artikel/'.$photo_name);
						$_SESSION['artikel_works_success'] = "Added Successfully!";
