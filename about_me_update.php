...
if(isset($_POST['submit'])){
	...
	if(!empty($name) &&!empty($intro) && !empty($details) && !empty($fb_link) && !empty($instagram_link) && !empty($youtube_link) && !empty($github_link) ){

		$work_insert = $dbcon->query("UPDATE about_me SET name='$name', nim='$nim', intro='$intro',details='$details',fb_link='$fb_link',instagram_link='$instagram_link',youtube_link='$youtube_link',github_link='$github_link' WHERE id=1");
		if($work_insert){
